...
(struct vgraphics * ctx, struct gft_char c[128], struct gft_atlas * atlas, float x, float y, float z, float sx, float sy, float l, float line_distance, char const * text)
{
	ASSERT_PARAM_NOTNULL (ctx);
	...
	uint32_t n;
	n = gft_gen_pos (v->xyzw.e, ctx->capacity, stride, text, c, x, y, sx, sy, line_distance);
	gft_gen_uv  (v->uvl.e, ctx->capacity, stride, text, c, atlas->w, atlas->h);
...
(struct vgraphics * ctx, struct gft_char c[128], struct gft_atlas * atlas, float x, float y, float z, float sx, float sy, float l, float line_distance, const char* format, ...)
{
	ASSERT_PARAM_NOTNULL (ctx);
	...
	//printf ("%s", buf);
	vgraphics_drawtext (ctx, c, atlas, x, y, z, sx, sy, l, line_distance, buf);
	va_end (va);
}
